...
def send_req(request):
    if request.method == 'GET':
        ...
        headers = request.GET.get('header', '')
        ...
        print('per_type-------------',per_type)
        print('per_value-------------',per_value)
        if method == "get":
            r = requests.get(url, params=per_value, headers=json_headers)
        elif method == "post":
            if per_type == "form":
                r = requests.post(url, data=per_value, headers=json_headers)
            else:
                r = requests.get(url, json=per_value, headers=json_headers)
